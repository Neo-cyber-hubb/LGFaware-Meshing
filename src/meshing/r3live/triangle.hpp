...
class Triangle
{
  public:
    ...
    int    m_index_flip = 0;  // 0-不需要翻转，当前已经是逆时针顺序
    int    rec_source = 0;    // 0-非平面 >=1-平面
...
//            unlock();
//            return false;
//            std::cout << "\033[31m No this triangle! \033[0m" << endl;
        }
        m_if_required_synchronized = true;
        unlock();
    }
...
//    Global_map*                             m_pointcloud_map = nullptr;
    Hash_map_3d< int, Triangle_ptr >        m_triangle_hash;              // 三角形有序点索引-》三角形指针，用来判断三角形是否存在
    double                                  m_region_size = 10.0;
    std::vector< Sync_triangle_set* >            m_triangle_set_vector;   // 所有区域对应set的指针列表，顺序无所谓
    // Hash_map_3d< int, Triangle_set >        m_triangle_set_in_region;
    Hash_map_3d< int, Sync_triangle_set >        m_triangle_set_in_region;   // 保存每个区域的三角形set

    std::unordered_map< int, Triangle_set > m_map_pt_triangle;  // 点-三角形索引
    ...
    void erase_triangle( const Triangle_ptr& tri_ptr )
    {
        ...
        erase_triangle_from_list( tri_ptr, m_newest_rec_frame_idx );   // 从所属体素的三角形列表中删除该三角形
        ...
        for ( int tri_idx = 0; tri_idx < 3; tri_idx++ )
        {
            ...
            {
                m_map_pt_triangle[ idx[ tri_idx ] ].erase( it3 );   // 删除点对该三角形的索引
            }
        }
        if ( m_enable_map_edge_triangle )  // =0
        {
            // printf_line;
            // erase triangle in edge-triangle list  边-三角形索引
        ...
        for ( typename std::set< T >::iterator it = set_index.begin(); it != set_index.end(); it++ )
        {
            if ( m_map_pt_triangle.find( *it ) != m_map_pt_triangle.end() )
            {
                for ( Triangle_set::iterator tri_it = m_map_pt_triangle[ *it ].begin(); tri_it != m_map_pt_triangle[ *it ].end(); tri_it++ )
                {
                    ...
                    {
                        triangle_ptr_list.insert( *tri_it );   // 点对应的三角形的三个点都包含在点集中
                    }
                }
            }
        }
    ...
    Triangle_ptr insert_triangle( int id_0, int id_1, int id_2, int build_triangle_map = false, const int & frame_idx = 0 )
    {
        ...
        ids[ 1 ] = id_1;
        ids[ 2 ] = id_2;
        std::sort( std::begin( ids ), std::end( ids ) );  // 三个点索引从小到大排列，作为唯一索引，指向对应三角形的指针
        ...
        if ( build_triangle_map )
        {
            ...
            m_map_pt_triangle[ ids[ 0 ] ].insert( triangle_ptr );   // 添加点-三角形索引

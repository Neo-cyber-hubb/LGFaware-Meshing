...
//                    normal_a << pts_list->points[it].normal_x, pts_list->points[it].normal_y, pts_list->points[it].normal_z;
//                    normal_b << pts_list->points[idx].normal_x, pts_list->points[idx].normal_y, pts_list->points[idx].normal_z;
                    double dis = (pt_a - pt_b).norm();
//                    double ang = normal_a.dot(normal_b);
                    if (dis <= r_dis /*&& ang >= 0*/)
                    {
                        // 距离小于阈值，法向夹角不超过90度
                        idx_dis_dic[it] = dis;
...
//    double d = qp.dot(norm);
//    // 射线与三角形平行或者射线方向与三角形法向相反
//    if (d <= 0) { return false; }
...
//    {
//        std::cout << "\033[31m Error: pt2face_idx[pt_idx].size() = 0 \033[0m" << std::endl;
//    }
...
bool NonPlaneMesh::if_3d_vertex_visible(const int& P_idx, const int& Q_idx, const std::set<int>& faces)
{
    for (auto it : faces)
    {
        int A_idx = faces_list_all[it][0];
        int B_idx = faces_list_all[it][1];
        int C_idx = faces_list_all[it][2];
        Eigen::Vector3d A, B, C, P, Q;
        A << pts_list->points[A_idx].x, pts_list->points[A_idx].y,
                pts_list->points[A_idx].z;
        B << pts_list->points[B_idx].x, pts_list->points[B_idx].y,
                pts_list->points[B_idx].z;
        C << pts_list->points[C_idx].x, pts_list->points[C_idx].y,
                pts_list->points[C_idx].z;
        P << pts_list->points[P_idx].x, pts_list->points[P_idx].y,
                pts_list->points[P_idx].z;
        Q << pts_list->points[Q_idx].x, pts_list->points[Q_idx].y,
                pts_list->points[Q_idx].z;
        ...
        double t = ap.dot(norm);
        // 射线起始点P在三角形背面
        if (t < 0) { continue; }
        // d是射线沿三角形法向的投影长度，t是射线起始点P到三角形所在平面的距离，t>d说明射线终点Q在三角形正面，两者不相交
        if (t > d) { continue; }

        Eigen::Vector3d e = qp.cross(ap);
        double v = ac.dot(e);

        // M = (1-u-v) * A + u * B + v * C，其中u = v / d，v = w / d，保证点ABC的权重均在0-1之间则点M在三角形内部
        if (v < 0 || v > d) { continue; }
        double w = -ab.dot(e);
...
//        pts_state.push_back(2);
//        pts_process_num.push_back(1);
//        tri_edge_v_list_all.push_back(pts_list->size() - 1);
//        pt2face_idx.resize(pts_list->size());
    }
    // 确定三角形和顶点顺序
...
//void NonPlaneMesh::edge_vertex_process(int current_idx, int v1_idx, int v2_idx)
//{
//
//}
...
void NonPlaneMesh::fill_hole(std::vector<int>& hole_pts_list)
{
    ...
    if (hole_pts_list.size() == 3)
    {
        // 根据邻接三角形确定法向
        int adj_tri_idx = -1;
//        for (auto it : pt2face_idx[hole_pts_list[1]])
//        {
//            for (int l = 0; l < 3; l++)
//            {
//                if (faces_list_all[it][l] == hole_pts_list[2])
//                {
//                    adj_tri_idx = it;
//                }
//            }
...
//            }
    ...
    {
        // debug nyq
//        pro_tri_list.insert(added_tri_list[j]);
//        faces_to_delete.push_back(added_tri_list[j]);
...
{
//    fprintf(f, "pts_list->size(): %d \n", pts_list->size());
//    std::cout << "tri_edge_v_list_all.size()  " << tri_edge_v_list_all.size() << std::endl;
//    fprintf(f, "tri_edge_v_list_all.size(): %d ", tri_edge_v_list_all.size());
    ...
    auto time_1 = std::chrono::duration_cast<std::chrono::duration<double>>(end_1 - end_0).count() * 1000;
//    fprintf(f, "t_1: %.4f ms ", time_1);
    ...
    auto time_2 = std::chrono::duration_cast<std::chrono::duration<double>>(end_2 - end_1).count() * 1000;
//    fprintf(f, "t_2: %.4f ms ", time_2);
//    fprintf(f, "hole_list.size(): %d ", hole_list.size());
    ...
    auto time_3 = std::chrono::duration_cast<std::chrono::duration<double>>(end_3 - end_2).count() * 1000;
//    fprintf(f, "t_3: %.4f ms ", time_3);
...
//    fprintf(f, "tri_edge_v_list_all.size(): %d ", tri_edge_v_list_all.size());
    ...
    auto time_4 = std::chrono::duration_cast<std::chrono::duration<double>>(end_4 - end_0).count() * 1000;
//    fprintf(f, "all: %.4f ms \n", time_4);
}



//void NonPlaneMesh::overlay_triangulation_process()
//{
//
//}

int NonPlaneMesh::abnormal_vertex_count()
{
    int num = 0;
    for (int i = 0; i < pt2face_idx.size(); i++)
    {
        if (pt2face_idx[i].size() == 0)
        {
            continue;
        }
        std::vector<int> adj_v_list, abnormal_v_list;
        find_edge_and_abnormal_vertex(i, adj_v_list, abnormal_v_list);
        if (abnormal_v_list.size() > 0)
        {
            num++;
        }
    }
...
{
//    fprintf(f, "voxel_to_update: %d \n", voxel_to_update.size());
...
//                    avg_normal << pts_list->points[current_idx].normal_x,
//                            pts_list->points[current_idx].normal_y,
//                            pts_list->points[current_idx].normal_z;
                    ...
                    auto end_3 = std::chrono::high_resolution_clock::now();
                    auto time_2 = std::chrono::duration_cast<std::chrono::duration<double>>(end_3 - end_2).count() * 1000;
//                    fprintf(f, "fit_2: %.4f ms ", time_2);
...
/*
                        // 关联三角形多于一个
                        if (pt2face_idx[nearest_vertexs[0]].size() < 2)
                        {
                            pts_state[current_idx] = 0; // TODO 搜索最近邻边缘点处理
                            continue;
                        }

                        // 判断最近顶点是否只有两个边缘点
                        std::vector<int> adj_edge_v_list;
                        find_edge_vertex(nearest_vertexs[0], adj_edge_v_list);
                        if (adj_edge_v_list.size() != 2)
                        {
                            pts_state[current_idx] = 0; // TODO 0
                            continue;
                        }

                        // 判断当前点与两个边缘点组成的向量之间的夹角
                        Eigen::Vector3d centerv, v1, v2, c_v1, c_v2, v1_2, cur_pt_eigen;
                        cur_pt_eigen << pts_list->points[current_idx].x, pts_list->points[current_idx].y,
                                pts_list->points[current_idx].z;
                        centerv << pts_list->points[nearest_vertexs[0]].x, pts_list->points[nearest_vertexs[0]].y,
                                pts_list->points[nearest_vertexs[0]].z;
                        ...
                        // 大于180度 -》当前点是否在最近顶点和两个边缘点组成的三角形内部
                        if (!if_p_in_tri(cur_pt_eigen, centerv, v1, v2))
                        {
                            pts_state[current_idx] = 0; // TODO 0
                        }
                        else
                        {
                            ...
                            if (cos_c_v1_v2 < 0)
                            {
                                ...
                                if (target_tri != -1)
                                {
                                    ...
                                    {
                                        std::array<int, 3> temp_add_t;
                                        temp_add_t[0] = nearest_vertexs[0];
                            ...
                            else if (cos_c_v1_v2 > 0.93969)
                            {
                                ...
                                // 找要删除的三角形索引
                                int target_tri = -1;
                                for (auto it : pt2face_idx[nearest_vertexs[0]])
                                {
                                    for (int l = 0; l < 3; l++)
                                    {
                                ...
                                // 确定第四个点
                                int v3_idx;
                                if (A_idx != nearest_vertexs[0] && A_idx != min_ang_pt_idx)
                                {
                                    v3_idx = A_idx;
                                }
                                else if (B_idx != nearest_vertexs[0] && B_idx != min_ang_pt_idx)
                                {
                                    v3_idx = B_idx;
                                }
                                else
                                ...
                                if (if_acw_nc)
                                {
                                    std::array<int, 3> temp_add_t;
                                    temp_add_t[0] = v3_idx;
                                    temp_add_t[1] = nearest_vertexs[0];
                                ...
                                else
                                {
                                    ...
                                    temp_add_t[2] = nearest_vertexs[0];
                                    tri_idx_add.push_back(faces_list_all.size());
                                    faces_list_all.push_back(temp_add_t);
                                }
                                // 第二个三角形
                            ...
                            else
                            {
                                ...
                                for (int nvi = 1; nvi < nearest_vertexs.size(); nvi++)
                                {
                                    ...
                                    if (if_p_in_tri(temp_v, centerv, v1, v2))
                                    {
                                        if_any_v_inner = true;
                                        break;
                                    }
                            ...
                            if (tri_idx_add.size() > 0)
                            {
                                pts_state[current_idx] = 3;   // 删除状态
                            }
                            else
                            {
                                pts_state[current_idx] = 0;   // 删除状态
                            }
                        }*/
                    }
                    else
...
//                                edge_tri_id_list[temp_idx].push_back(it);
                                }
                            }
                        }
                        // 确定边缘边，格式 A-B-C
...
//                                fprintf(f, "ifc_%d ", if_continuous);
...
//                                    fprintf(f, "nocontinuous ");
                    ...
                    auto time_3 = std::chrono::duration_cast<std::chrono::duration<double>>(end_5 - end_4).count() * 1000;
//                    fprintf(f, "extend: %.4f ms ", time_3);
                }
...
//            fprintf(f, "\n");
        ...
        for (int i = 0; i < voxel_pts.m_map_3d_hash_map[voxel_x][voxel_y][voxel_z]->all_pts_idx.size(); i++)
        {
            ...
            mtx_pts_process_num.unlock();
/*
            std::vector<int> edge_v_list;
            find_edge_vertex(current_idx, edge_v_list);
            if (edge_v_list.size() == 0)
            {
                pts_state[current_idx] = 1;
            }
            if (edge_v_list.size() == 2)
            {
                // 判断是否存在包含另外两个边缘点的三角形
                int target_tri = -1;
                for (auto it : pt2face_idx[edge_v_list[0]])
                {
                    for (int l = 0; l < 3; l++)
                    {
                        if (faces_list_all[it][l] == edge_v_list[1])
                        {
                            target_tri = it;
                            break;
                        }
                ...
                if (target_tri == -1) { continue; }

                Eigen::Vector3d cur_pt_eigen, v1, v2, cur_v1, cur_v2;
                cur_pt_eigen << pts_list->points[current_idx].x, pts_list->points[current_idx].y,
                        pts_list->points[current_idx].z;
                v1 << pts_list->points[edge_v_list[0]].x, pts_list->points[edge_v_list[0]].y,
                        pts_list->points[edge_v_list[0]].z;
                v2 << pts_list->points[edge_v_list[1]].x, pts_list->points[edge_v_list[1]].y,
                        pts_list->points[edge_v_list[1]].z;
                cur_v1 = v1 - cur_pt_eigen;
                cur_v2 = v2 - cur_pt_eigen;

                double cos_c_v1_v2 = cur_v1.dot(cur_v2) / cur_v1.norm() / cur_v2.norm();
                std::vector<int> tri_idx_add, tri_idx_delete;
                // 夹角过大 140 - 180
                if (cos_c_v1_v2 < -0.766)
                {
                    faces_to_delete.push_back(target_tri);
                    tri_idx_delete.push_back(target_tri);
                    // 要删除的三角形的三个点，计算法向
                    ...
                    // 确定第四个点
                    int v3_idx;
                    if (A_idx != edge_v_list[0] && A_idx != edge_v_list[1])
                    {
                        v3_idx = A_idx;
                    }
                    else if (B_idx != edge_v_list[0] && B_idx != edge_v_list[1])
                    {
                        v3_idx = B_idx;
                    }
                    else
                    ...
                    else
                    {
                        ...
                        temp_add_t[0] = current_idx;
                        temp_add_t[1] = v3_idx;
                        temp_add_t[2] = edge_v_list[0];
                        tri_idx_add.push_back(faces_list_all.size());
                        faces_list_all.push_back(temp_add_t);
                    }
                    // 第二个三角形
                    n = cur_v2.cross(cur_v3);
                    if_acw_nc = true;
                    if (n.dot(tri_n) < 0)
                    {
                        if_acw_nc = false;
...
//            find_edge_vertex(current_idx, adj_edge_v_list);
            find_edge_and_abnormal_vertex(current_idx, adj_edge_v_list, abnormal_v);
//            fprintf(f, "\n %d  %d \n", adj_edge_v_list.size(), abnormal_v.size());
//            if (adj_edge_v_list.size()%2 == 1 || abnormal_v.size() > 0 )
//            {
//                for (auto idx : pt2face_idx[current_idx])
//                {
//                    pro_tri_list.insert(idx);
//                }
//            }
            ...
            if (adj_edge_v_list.size() == 0)
            {
                ...
                {
//                    std::cout << "-----------------" << std::endl;
//                    int ti1 = pt2face_idx[current_idx][0];
//                    int ti2 = pt2face_idx[current_idx][1];
//                    std::cout << current_idx << std::endl;
//                    std::cout << faces_list_all[ti1][0] << " " << faces_list_all[ti1][1] << " " << faces_list_all[ti1][2] << std::endl;
//                    std::cout << faces_list_all[ti2][0] << " " << faces_list_all[ti2][1] << " " << faces_list_all[ti2][2] << std::endl;
...
//                    if (tri_idx_add.size() > 0)
//                    {
//                        pts_state[current_idx] = 1;   // 删除状态
//                    }
//                    else
//                    {
//                        pts_state[current_idx] = 2;   // 删除状态
//                    }
                }
            }

            auto end_7 = std::chrono::high_resolution_clock::now();
            auto time_4 = std::chrono::duration_cast<std::chrono::duration<double>>(end_7 - end_6).count() * 1000;
//            fprintf(f, "edge_process: %.4f ms \n", time_4);
        }
...
//        return true;
    ...
    else
    {
        ...
        if (std::abs(pt_pro_plane(0)) > 0.2)  // TODO 阈值
        {
            // 虽投影在平面内，但与平面法向距离太大
            return false;
        }
...
{
//    std::cout << "plane->pts_edge_delete  " << plane->pts_edge_delete.size() << std::endl;
//    std::cout << "plane->pts_inner_add  " << plane->pts_inner_add.size() << std::endl;
//    std::cout << "plane->pts_edge_add  " << plane->pts_edge_add.size() << std::endl;
//    std::cout << plane->plane_idx << "  " << plane->plane_np << "  ";
//    std::cout << plane->pts_edge_delete.size() << "  " << plane->pts_edge_add.size();
//    std::cout << std::endl;
//    plane->plane_np++;
...
//    }
//    std::cout << std::endl;
//    for (auto pt_idx : plane->pts_edge_add)
//    {
//        std::cout << pt_idx << "  ";
//    }
//    std::cout << std::endl;
...
//            auto ittt = pro_tri_list.find(tri_idx);
//            if (ittt != pro_tri_list.end()) {
//                pro_tri_list.erase(ittt);
//            }
...
//    auto end_0 = std::chrono::high_resolution_clock::now();
//    auto time_0 = std::chrono::duration_cast<std::chrono::duration<double>>(end_0 - total_start).count() * 1000;
//    fprintf(f, "delete_edge: %.4f ms ", time_0);
...
//    auto end_1 = std::chrono::high_resolution_clock::now();
//    auto time_1 = std::chrono::duration_cast<std::chrono::duration<double>>(end_1 - end_0).count() * 1000;
//    fprintf(f, "delete_re: %.4f ms ", time_1);
...
//    auto end_2 = std::chrono::high_resolution_clock::now();
//    auto time_2 = std::chrono::duration_cast<std::chrono::duration<double>>(end_2 - end_1).count() * 1000;
//    fprintf(f, "delete_inner: %.4f ms ", time_2);
...
//        pro_tri_list.insert(faces_list_all.size() - 1);
...
//    fprintf(f, "add_tri: %.4f ms ", time_5);
//
//    auto end_6 = std::chrono::high_resolution_clock::now();
//    auto time_6 = std::chrono::duration_cast<std::chrono::duration<double>>(end_6 - total_start).count() * 1000;
//    fprintf(f, "total: %.4f ms \n", time_6);
}
...
{
//    std::cout << "plane_edge_v_list_all.size() " << plane_edge_v_list_all.size() << std::endl;
...
//        for (auto tri_idx : pt2face_idx[cur_edge_v_idx])
//        {
//            pro_tri_list.insert(tri_idx);
//        }
...
//    PointCloudXYZI::Ptr pts2vertex_list(new PointCloudXYZI());
//    std::unordered_map<int, int> pt_old2new_idx;
//    for (int i = 0; i < pts_list->size(); i++)
//    {
//        if (pts_state[i] == 1)
//        {
//            pt_old2new_idx[i] = pts2vertex_list->size();
//            pts2vertex_list.push_back(pts_list->points[i]);
//        }
//    }
//    pcl::toPCLPointCloud2(*pts2vertex_list, mesh.cloud);
...
/*
    int num = 0;
    for(auto it : voxel_pts.m_map_3d_hash_map)
    {
        for(auto it_it: it.second)
        {
            for( auto it_it_it: it_it.second )
            {
                for (auto tri : it_it_it.second->face_list)
                {
                    pcl::Vertices vertices;
                    vertices.vertices.push_back(tri[0]);   // meshlab点序逆时针
                    vertices.vertices.push_back(tri[1]);
                    vertices.vertices.push_back(tri[2]);
                    mesh.polygons.push_back(vertices);
                    num++;
                }
            }
...
void NonPlaneMesh::mtri_save_to_ply(const std::string &fileName)
{
    pcl::PolygonMesh mesh;
    mesh.header = pcl::PCLHeader();
    mesh.cloud = pcl::PCLPointCloud2();
    ...
    for (auto tri_idx : pro_tri_list)
    {
        pcl::Vertices vertices;
        vertices.vertices.push_back(faces_list_all[tri_idx][0]);   // meshlab点序逆时针
        vertices.vertices.push_back(faces_list_all[tri_idx][1]);
        vertices.vertices.push_back(faces_list_all[tri_idx][2]);
        mesh.polygons.push_back(vertices);
    }

    int num = pro_tri_list.size();
    std::cout << "The number of my np triangulation is " << num << std::endl;

    pcl::io::savePLYFile(fileName, mesh);
}

void NonPlaneMesh::mesh_extract_to_debug()
{
    std::cout << "plane_edge_v_list_all.size() " << plane_edge_v_list_all.size() << std::endl;
    for (int i = 0; i < plane_edge_v_list_all.size(); i++)
    {
...
//        find_edge_vertex(cur_edge_v_idx, adj_edge_v_list);
//
//        std::cout << adj_edge_v_list.size() << std::endl;
        for (auto tri_idx : pt2face_idx[cur_edge_v_idx])
        {
            pro_tri_list.insert(tri_idx);
        }

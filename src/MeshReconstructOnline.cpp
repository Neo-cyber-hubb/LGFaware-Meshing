...
int main(int argc, char **argv) {
    ...
    ptcl_save_path = ptcl_save_path + "/" + file_name_str + "/";
    ...
    // 发布器
    ros::Publisher pubLaserCloudFull = m_node.advertise<sensor_msgs::PointCloud2>("/republish", 100000);
...
//    string log_save_path1 = ptcl_save_path + "debug1.txt";
//    fp1 = fopen(log_save_path1.c_str(), "w");
//    string log_save_path2 = ptcl_save_path + "debug2.txt";
//    fp2 = fopen(log_save_path2.c_str(), "w");
    string log_save_path3 = ptcl_save_path + "log_time.txt";
    fpTime = fopen(log_save_path3.c_str(), "w");

//    string log_save_path4 = ptcl_save_path + "mesh_time.txt";
//    fpTime_mesh = fopen(log_save_path4.c_str(), "w");
//    string log_save_path5 = ptcl_save_path + "np_time_1.txt";
//    fpTime_np_1 = fopen(log_save_path5.c_str(), "w");
//    string log_save_path6 = ptcl_save_path + "np_hole.txt";
//    fpTime_hole = fopen(log_save_path6.c_str(), "w");

//    fpTime_np_3 = fopen("/home/neo/ws_onlinemesh/src/VoxelMap/debug_nyq/np_time_3.txt", "w");
    bool is_write = true;
    string bin_save_path = ptcl_save_path + "bin";

    std::vector<cv::Vec3b> colorLib(10);   // BGR
    colorLib[0] = cv::Vec3b(255, 0, 0);
    colorLib[1] = cv::Vec3b(0, 255, 0);
    colorLib[2] = cv::Vec3b(0, 0, 255);
    colorLib[3] = cv::Vec3b(255, 255, 255);
    colorLib[4] = cv::Vec3b(255, 255, 0);   // 青色
    colorLib[5] = cv::Vec3b(255, 0, 255);   // 品红
    colorLib[6] = cv::Vec3b(0, 255, 255);   // 黄色
    colorLib[7] = cv::Vec3b(203, 192, 255); // 粉色
    colorLib[8] = cv::Vec3b(0, 165, 255);   // 橙色
    colorLib[9] = cv::Vec3b(34, 139, 34);   // 森林绿色
...
//            std::cout << "\033[31m Error: buffer is empty! \033[0m" << std::endl;
            continue;
        }
...
//            u = std::round(-1 * p_body(1) / p_body(0) * W / 2 * 90 / angle_horizon + W / 2);
//            v = std::round(-1 * p_body(2) / p_body(0) * H / 2 * 90 / angle_vertical + H / 2);
...
//                        ptcl_frame->points[i].x, ptcl_frame->points[i].y, ptcl_frame->points[i].z,
...
//        cout << "Project cost time is " << time_1 << " ms!" << endl;
...
//        cout << "Normal cost time is " << time_3 << " ms!" << endl;
        ...
        for ( int i = r_kNN+1; i < H-r_kNN-1; i++ )
        {
            ...
            {
//                if (is_write) {
//                    fprintf(fp1, "%.4f %.4f %.4f %.4f \n", normalImage(i, j, 0), normalImage(i, j, 1),
//                            seg_result(i, j), eigenvalue_threshold);
//                }
...
//                    if (abs(normalImage(k, l, 0)) > eigenvalue_threshold)  // 是否严格控制种子点？
//                    {
//                        continue;
//                    }

//                    // 更新平面法向、平面中心
//                    if (normalImage(k, l, 0) > 0 && normalImage(k, l, 0) < eigenvalue_threshold && N_confidence < 20)
//                    {
//                        norm_ij(0) = (norm_ij(0) * N_confidence + normalImage(i, j, 1)) / (N_confidence + 1);
//                        norm_ij(1) = (norm_ij(1) * N_confidence + normalImage(i, j, 2)) / (N_confidence + 1);
//                        norm_ij(2) = (norm_ij(2) * N_confidence + normalImage(i, j, 3)) / (N_confidence + 1);
//                        seg_center(0) = (seg_center(0) * N_confidence + projectImage(i, j, 0)) / (N_confidence + 1);
//                        seg_center(1) = (seg_center(1) * N_confidence + projectImage(i, j, 1)) / (N_confidence + 1);
//                        seg_center(2) = (seg_center(2) * N_confidence + projectImage(i, j, 2)) / (N_confidence + 1);
//                        N_confidence++;
//                    }

//                    Eigen::Vector3d norm_kl, seg_center_kl;
//                    norm_kl(0) = normalImage(k, l, 1);
//                    norm_kl(1) = normalImage(k, l, 2);
//                    norm_kl(2) = normalImage(k, l, 3);
//                    seg_center_kl(0) = projectImage(k, l, 0);
//                    seg_center_kl(1) = projectImage(k, l, 1);
//                    seg_center_kl(2) = projectImage(k, l, 2);
...
//                            Eigen::Vector3d p2q_2;
//                            p2q_2(0) = seg_center_kl(0) - projectImage(m, n, 0);
//                            p2q_2(1) = seg_center_kl(1) - projectImage(m, n, 1);
//                            p2q_2(2) = seg_center_kl(2) - projectImage(m, n, 2);
//                            if (abs(p2q_2.dot(norm_ij)) > depth_threshold) { continue; }
        ...
        /****** 将每帧平面点云保存成.bin文件 ******/
//        pcl::PointCloud<pcl::PointXYZI>::Ptr bin_cloud(new pcl::PointCloud<pcl::PointXYZI>);
...
//                temp_pt.x = projectImage(m, n, 0);
//                temp_pt.y = projectImage(m, n, 1);
//                temp_pt.z = projectImage(m, n, 2);
//                temp_pt.intensity = 0;
//                bin_cloud->points.push_back(temp_pt);
//            }
//        }
//        std::stringstream bin_point_cloud_path;
//        bin_point_cloud_path << bin_save_path << "/" << std::setfill('0') << std::setw(5) << frame_idx << ".bin";
//        saveCloudToBin(bin_cloud, bin_point_cloud_path.str());
...
//                        for ( int j = 1; j < same_plane_list.size(); j++ )
//                        {
//                            int temp_key = plane_map[same_plane_list[j]]->plane_idx;
//                            auto temp_it = plane_idx2ptr.find(temp_key);
...
//        std::cout << "np_hash_2" << std::endl;

        // 逐体素、逐点处理，孤立体素新建mesh，体素内的点先处理法向投影在三角形内部的，再处理投影在外部
        // 对于投影在外部的点，根据边缘点查找最近然后扩展该点
//        std::cout << voxel_to_update.size() << std::endl;
//        non_plane_map->mesh_update(voxel_to_update, pos_vec);

        // 并行处理体素块
//        std::cout << "voxel_to_update_parallel.size: " << voxel_to_update_parallel.size() << std::endl;
...
//        std::cout << "np_mesh_p" << std::endl;

        // 最后处理间隔区域
//        std::cout << "voxel_to_update_last.size: " << voxel_to_update_last.size() << std::endl;
...
//        std::cout << "np_mesh_l" << std::endl;

        // 处理空洞
        if (frame_idx % hole_every_n_frame == 0)
        {
//            std::cout << "mesh_hole_process" << std::endl;
...
//        std::cout << "np_hole" << std::endl;

        // 连接平面非平面
        if (frame_idx % hole_every_n_frame == 0)
        {
//            std::cout << "plane_and_noplane_connect" << std::endl;
...
//        std::cout << "gui" << std::endl;
        ...
        if (frame_idx % 10 == 1 && 0)
        {
            cv::Mat saveImage = cv::Mat( cv::Size(W, H), CV_8UC3, cv::Scalar(50, 50, 50));
            int real_plane_num = 0;

            for ( int i = 0; i < plane_points_uv.size(); i++ )
            {
//                if ( plane_points_uv[i].size() < ptsNum_threshold ) { continue; }    // 点数太少不处理

                real_plane_num++;
                int colorK = i % colorLib.size();
                for (int j = 0; j < plane_points_uv[i].size(); j++)
                {
                    int uu = plane_points_uv[i][j](0);
                    int vv = plane_points_uv[i][j](1);
//                saveImage.at<cv::Vec3b>(uu , vv) = cv::Vec3b(0, 0, 255);
                    saveImage.at<cv::Vec3b>(uu , vv) = colorLib[colorK];
                }
            }
...
//                for (int j = 0; j < W; j++)
//                {
//                    if ( normalImage(i, j, 0) >= 0  )
//                    {
//                        if ( normalImage(i, j, 0) < eigenvalue_threshold )
//                        {
//                            saveImage.at<cv::Vec3b>(i , j) = cv::Vec3b(0, 0, 255);
//                        }
//                        else
//                        {
//                            saveImage.at<cv::Vec3b>(i , j) = cv::Vec3b(255, 255, 255);
//                        }
//                    }
//                }
//            }

            image_name = to_string(frame_idx);
            image_save_path = ptcl_save_path + "imgs_5/" + image_name + ".jpg";
            cv::imwrite(image_save_path, saveImage);
        }
...
//            fclose(fp2);
        ...
        /**************** republish ****************/
//        PointCloudXYZI::Ptr pcl_normal_frame(new PointCloudXYZI());
//        if (frame_idx < 1550 || frame_idx > 1800)
//        {
//            for (int i_ = 0; i_ < H; i_++)
//            {
//                for (int j_ = 0; j_ < W; j_++)
//                {
//                    if (normalImage(i_, j_, 0) == -1) {continue;}
//                    Eigen::Vector3d p_body;
//                    p_body(0) = projectImage(i_, j_, 0);
//                    p_body(1) = projectImage(i_, j_, 1);
//                    p_body(2) = projectImage(i_, j_, 2);
//                    Eigen::Vector3d p_global(rot_mat * (p_body) + pos_vec);
//
//                    pcl::PointXYZINormal temp_pt;
//                    temp_pt.x = p_global(0);
//                    temp_pt.y = p_global(1);
//                    temp_pt.z = p_global(2);
//                    temp_pt.normal_x = normalImage(i_, j_, 1);
//                    temp_pt.normal_y = normalImage(i_, j_, 2);
//                    temp_pt.normal_z = normalImage(i_, j_, 3);
////                pcl_normal_frame->points.push_back(temp_pt);
//                    pcl_normal->points.push_back(temp_pt);
//                }
//            }
//        }

//        sensor_msgs::PointCloud2 laserCloudmsg;
//        pcl::toROSMsg(*pcl_normal_frame, laserCloudmsg);
...
//    merged_plane->save_to_ply(save_dir);
//    merged_plane->mtri_save_to_ply(mp_save_dir);
    if (if_debug)
    {
        // 去除不连接三角形的点
...
//    int num = 0;
//    pcl::PCDWriter pcd_writer;
//    for (auto mf : plane_map)
//    {
//        string pts_name = to_string(num);
//        string save_dir = ptcl_save_path + "plane_mesh_1/" + pts_name + ".ply";
//        string save_dir_1 = ptcl_save_path + "plane_pcd_1/" + pts_name + ".pcd";
//        if ( mf->ptcl_grid->points.size() > 0)
//        {
////            mf->save_to_ply(save_dir);
////            mf->mtri_save_to_ply(save_dir);
//            pcd_writer.writeBinary(save_dir_1, *(mf->ptcl_all));
//
//        }
//        else
//        {
...
//    PointCloudXYZI::Ptr plane_ptcl_save(new PointCloudXYZI());
//    for (auto mf : plane_map)
//    {
//       *plane_ptcl_save += *mf->ptcl_all;
//    }
//
//    string save_dir_1 = ptcl_save_path + "plane_points.pcd";
////    string save_dir_2 = ptcl_save_path + "edge_points.pcd";
//    pcd_writer.writeBinary(save_dir_1, *plane_ptcl_save);
////    pcd_writer.writeBinary(save_dir_2, *edge_ptcl_save);
...
//                                } );
        ...
        if (if_debug)
        {
            ...
            string save_dir_pdebug = ptcl_save_path + "p_debug.ply";
            merged_plane_1->mtri_save_to_ply(save_dir_pdebug);
        }
    }

    /****** 保存非平面mesh、自定义mesh、点云 ******/
//    non_plane_map->mesh_extract_to_debug();
    string mesh_save_dir = ptcl_save_path + "np.ply";
    string m_mesh_save_dir = ptcl_save_path + "m_np.ply";
    string pc_save_dir = ptcl_save_path + "np.pcd";
//    pcd_writer.writeBinary(pc_save_dir, *(non_plane_map->pts_list));

    if (if_debug)
    {
        //    non_plane_map->save_to_ply(mesh_save_dir);
        //    non_plane_map->mtri_save_to_ply(m_mesh_save_dir);
        // 去除不连接三角形的点
...
//    int abn_v_num = non_plane_map->abnormal_vertex_count();
//    std::cout << "abnormal_vertex_count  " << abn_v_num << std::endl;

//    string pc_rest_save_dir = ptcl_save_path + "np_rest.pcd";
//    pcd_writer.writeBinary(pc_rest_save_dir, *(pts_list_rest));

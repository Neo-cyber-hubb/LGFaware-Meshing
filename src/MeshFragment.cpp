...
void MeshFragment::grid_decimate(double dis_resol, FILE *fpTime_mesh)
{
    ...
    if (ptcl_all->size() > ptsNum_thres)   // 只有点的数量大于阈值才执行降采样和构建mesh
    {
        ...
        if ( if_first_decimate )
        {
            ...
            double y_start = y0 - dis_resolution * leftW;
            double z_start = z0 + dis_resolution * topH;
...
//        std::vector<std::vector<int>> uv_list;
...
//            std::vector<int> temp_uv;
//            temp_uv.push_back(u);
//            temp_uv.push_back(v);
//            uv_list.push_back(temp_uv);
...
//            vertex_and_face_list_update();
        ...
        else
        {
            ...
            double y_start = y0 - dis_resolution * new_leftW;
            double z_start = z0 + dis_resolution * new_topH;

            auto end_6 = std::chrono::high_resolution_clock::now();
            auto time_6 = std::chrono::duration_cast<std::chrono::duration<double>>(end_6 - end_0).count() * 1000;
//            fprintf(fpTime_mesh, "project: %.4f ms ", time_6);
            ...
            auto time_1 = std::chrono::duration_cast<std::chrono::duration<double>>(end_1 - end_6).count() * 1000;
//            fprintf(fpTime_mesh, "grid: %.4f ms ", time_1);
...
//            fprintf(fpTime_mesh, "update: %.4f ms ", time_2);
...
//            fprintf(fpTime_mesh, "mc_mesh: %.4f ms ", time_3);
...
//            fprintf(fpTime_mesh, "face_update: %.4f ms", time_4);

            auto time_5 = std::chrono::duration_cast<std::chrono::duration<double>>(end_4 - end_0).count() * 1000;
//            fprintf(fpTime_mesh, "total: %.4f ms\n", time_5);
        }
    }
}
...
void MeshFragment::MC_mesh()
{
    // 提取mesh的三角面片   TODO 并行处理 可以不是每帧都提取mesh
//    if ( !if_first_decimate )
//    {
//
//    }
    ...
    for (int u = 0; u < H; u++)
    {
        for (int v = 0; v < W; v++)
        {
            if (!uniform_grid[u][v].if_grid_need_update) {continue;}
            ...
            // 判断内部还是边缘
            bool if_inside = true;
            Eigen::Vector4i grid_vertex_state = {1, 1, 1, 1};   // 左上角开始 顺时针
            for (int j = 0; j < 9; j++)
            {
                if (j == 4) {continue;}
...
// 每次重新提取点列表、面列表 TODO 每次重新提取，地面点范围大比较耗时，改为增量提取
// 首先提取点列表，要求不同网格公用顶点坐标一致，然后根据每个网格的三角形顶点列表中点的数量(3 4 5 6)构建三角剖分
void MeshFragment::vertex_and_face_list_extract()
{
//    if ( !if_first_decimate )
//    {
//
//    }
    ...
    Eigen::MatrixXi grid_vertex_idx(2*H+1, 2*W+1);
    grid_vertex_idx.setConstant(-1);

    for (int u = 0; u < H; u++)
    {
        for (int v = 0; v < W; v++)
        {
            if (uniform_grid[u][v].vertex_list.size() == 0)
            {
                continue;
            }
            ...
            for (int i = 0; i < uniform_grid[u][v].vertex_list.size(); i++)
            {
                ...
                if (grid_vertex_idx(idx_u, idx_v) == -1)
                {
                    ...
                    if (if_grid_vertex)
                    {
                        ...
                        if (idx_u == 0)
                        {
                            if (idx_v == 0)
                            {
                                temp_pt.x = uniform_grid[0][0].x_avg;
                            }
                            else if (idx_v == 2*W)
                            {
                                temp_pt.x = uniform_grid[0][W-1].x_avg;
                            }
                            else
                            {
                                temp_pt.x = (uniform_grid[0][idx_v/2].x_avg
                        ...
                        else if (idx_u == 2*H)
                        {
                            if (idx_v == 0)
                            {
                                temp_pt.x = uniform_grid[H-1][0].x_avg;
                            }
                            else if (idx_v == 2*W)
                            {
                                temp_pt.x = uniform_grid[H-1][W-1].x_avg;
                            }
                            else
                            {
                                temp_pt.x = (uniform_grid[H-1][idx_v/2].x_avg
                        ...
                        else
                        {
                            if (idx_v == 0)
                            {
                                temp_pt.x = (uniform_grid[idx_v/2][0].x_avg
                                             + uniform_grid[idx_v/2-1][0].x_avg) / 2;
                            }
                            else if (idx_v == 2*W)
                            {
                                temp_pt.x = (uniform_grid[idx_v/2][W-1].x_avg
                                             + uniform_grid[idx_v/2-1][W-1].x_avg) / 2;
                            }
                            else
                            {
                                temp_pt.x = (uniform_grid[idx_u/2][idx_v/2].x_avg
                    ...
                    {
                        temp_pt.x = uniform_grid[u][v].x_avg;
                    }
                    temp_pt.y = uniform_grid[u][v].vertex_list[i](0);
                    temp_pt.z = uniform_grid[u][v].vertex_list[i](1);
                    ...
                    grid_vertex_idx(idx_u, idx_v) = ptcl_grid->size() - 1;
                    temp_ind_list.push_back(grid_vertex_idx(idx_u, idx_v));
                }
                else
                {
                    if (!if_grid_vertex)
                    {
                        ptcl_grid->points[grid_vertex_idx(idx_u, idx_v)].x += uniform_grid[u][v].x_avg;
                        ptcl_grid->points[grid_vertex_idx(idx_u, idx_v)].y += uniform_grid[u][v].vertex_list[i](0);
                        ptcl_grid->points[grid_vertex_idx(idx_u, idx_v)].z += uniform_grid[u][v].vertex_list[i](1);
                        ptcl_grid->points[grid_vertex_idx(idx_u, idx_v)].x /= 2;
                        ptcl_grid->points[grid_vertex_idx(idx_u, idx_v)].y /= 2;
                        ptcl_grid->points[grid_vertex_idx(idx_u, idx_v)].z /= 2;
                    }
                    temp_ind_list.push_back(grid_vertex_idx(idx_u, idx_v));
                }
            }
            ...
            if (uniform_grid[u][v].vertex_list.size() == 3)
            {
                ...
                temp_tri_idx[1] = temp_ind_list[1];
                temp_tri_idx[2] = temp_ind_list[2];
            ...
            else if (uniform_grid[u][v].vertex_list.size() == 4)
            {
                ...
                temp_tri_idx_1[1] = temp_ind_list[1];
                temp_tri_idx_1[2] = temp_ind_list[2];
                faces_list.push_back(temp_tri_idx_1);

                temp_tri_idx_2[0] = temp_ind_list[2];
                temp_tri_idx_2[1] = temp_ind_list[3];
                temp_tri_idx_2[2] = temp_ind_list[0];
            ...
            else if (uniform_grid[u][v].vertex_list.size() == 5)
            {
                ...
                temp_tri_idx_2[0] = temp_ind_list[2];
                temp_tri_idx_2[1] = temp_ind_list[3];
                temp_tri_idx_2[2] = temp_ind_list[4];
                faces_list.push_back(temp_tri_idx_2);

                temp_tri_idx_3[0] = temp_ind_list[0];
                temp_tri_idx_3[1] = temp_ind_list[2];
                temp_tri_idx_3[2] = temp_ind_list[4];
            ...
            else if (uniform_grid[u][v].vertex_list.size() == 6)
            {
                ...
                temp_tri_idx_3[1] = temp_ind_list[5];
                temp_tri_idx_3[2] = temp_ind_list[0];
                faces_list.push_back(temp_tri_idx_3);

                temp_tri_idx_4[0] = temp_ind_list[0];
                temp_tri_idx_4[1] = temp_ind_list[2];   // TODO 修改 3-》2
                temp_tri_idx_4[2] = temp_ind_list[4];
    ...
    // 点坐标转换到世界坐标系下
    for (int i = 0; i < ptcl_grid->size(); i++)
    {
        Eigen::Vector3d pt_local, pt_global;
        pt_local << ptcl_grid->points[i].x,
                ptcl_grid->points[i].y,
                ptcl_grid->points[i].z;
        pt_global = quadtree_axes * pt_local + quadtree_center;
        ptcl_grid->points[i].x = pt_global(0);
        ptcl_grid->points[i].y = pt_global(1);
        ptcl_grid->points[i].z = pt_global(2);
    }
}

// 增量更新点列表、三角形列表
void MeshFragment::vertex_and_face_list_update()
{
    ...
    for (int u = 0; u < H; u++)
    {
        for (int v = 0; v < W; v++)
        {
            if (!uniform_grid[u][v].if_mesh_need_add)
            {
                continue;
            }
            uniform_grid[u][v].if_mesh_need_add = false;
    ...
    for (auto it : faces_to_delete_frame)
    {
        ...
        auto ittt = pro_tri_list.find(it);
...
//        for (int l = 0; l < 3; l++)
//        {
//            if (pts_state[faces_list[it][l]] == 2)
//            {
//                temp_num++;
//            }
//        }
//        if (temp_num != 2) { continue; }

        pro_tri_list.insert(it);
...
//        {
//            for (int j = 0; j < uniform_grid[0].size(); j++)
//            {
//                if (grid_vertex_count[i][j] == 2)
//                {
//                    for (auto it : uniform_grid[i][j].tri_idx_list)
//                    {
//                        pro_tri_list_1.insert(it);
//                    }
//                    if (uniform_grid[i][j].tri_idx_list.size() == 0)
//                    {
//                        std::cout << "-----------------------" << std::endl;
//                    }
//                }
...
//        }
        ...
        // 统一整个平面的x值
//        Eigen::MatrixXd eigen_cloud(ptcl_grid->size(), 3);
//        for (int i = 0; i < ptcl_grid->size(); ++i)
//        {
//            eigen_cloud(i, 0) = ptcl_grid->points[i].x;
//            eigen_cloud(i, 1) = ptcl_grid->points[i].y;
//            eigen_cloud(i, 2) = ptcl_grid->points[i].z;
//        }
//        Eigen::MatrixXd projected_pts(ptcl_grid->size(), 3);
//        projected_pts = (eigen_cloud.rowwise() - quadtree_center.transpose()) * quadtree_axes;
//
//        double x_avg_global = projected_pts.col(0).sum() / projected_pts.rows();
//        for (int i = 0; i < ptcl_grid->size(); ++i)
//        {
//            projected_pts(i, 0) = x_avg_global;
//        }
//        eigen_cloud = (projected_pts * quadtree_axes.transpose()).rowwise() + quadtree_center.transpose();
//
//        for (int i = 0; i < ptcl_grid->size(); ++i)
//        {
//            ptcl_grid->points[i].x = eigen_cloud(i, 0);
//            ptcl_grid->points[i].y = eigen_cloud(i, 1);
//            ptcl_grid->points[i].z = eigen_cloud(i, 2);
//        }
...
void MeshFragment::vertex_and_face_list_merge(const std::shared_ptr<MeshFragment>& new_plane)
{
    ...
    *ptcl_grid += *(new_plane->ptcl_grid);
    ...
    // 合并感兴趣的三角形
    for (auto tri_idx : new_plane->pro_tri_list_1)
    {
        pro_tri_list_1.insert(tri_idx + last_tri_num);
    }
}
...
void MeshFragment::mtri_save_to_ply(const std::string &fileName)
{
    pcl::PolygonMesh mesh;
    mesh.header = pcl::PCLHeader();
    mesh.cloud = pcl::PCLPointCloud2();
    ...
    for (auto tri_idx : pro_tri_list_1)
    {
        pcl::Vertices vertices;
        vertices.vertices.push_back(faces_list[tri_idx][0]);   // meshlab点序逆时针
        vertices.vertices.push_back(faces_list[tri_idx][1]);
        vertices.vertices.push_back(faces_list[tri_idx][2]);
        mesh.polygons.push_back(vertices);
    }

    int num = pro_tri_list_1.size();
    std::cout << "The number of my triangulation is " << num << std::endl;

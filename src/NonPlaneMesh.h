...
public:
    ...
    // 点的状态 0-新加入的点 1-已完成np三角形顶点 2-np未完成/边缘顶点 3-要删除的点 4-平面内部点 5-平面边缘点 6-连接完成的平面边缘点
    std::vector<int> pts_state;
    ...
    double r_dis, minimum_pt_dis;
    int N_pts;
    std::set<int> pro_tri_list;
//    std::vector<int> pro_tri_list;
    int nnnnum = 0;
    std::vector<int> tri_edge_v_list_all;
//    std::set<int> tri_edge_v_list_all;
    ...
    double N_r_dis_for_edge_v = 1.0; // npp_connect查找平面边缘点最近的非平面边缘点
...
//    bool compare(const std::pair<int, double>& a, const std::pair<int, double>& b);
    ...
    // 没用到
    bool if_3d_vertex_visible(const int& P_idx, const int& Q_idx, const std::set<int>& faces);

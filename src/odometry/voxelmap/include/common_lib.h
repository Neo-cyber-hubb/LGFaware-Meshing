...
typedef online_mesh::Pose6D Pose6D;
...
struct StatesGroup {
  StatesGroup() {
    this->rot_end = M3D::Identity();     // 旋转
    this->pos_end = Zero3d;              // 位移
    this->vel_end = Zero3d;              // 速度
    this->bias_g = Zero3d;               // 角速度偏差
    this->bias_a = Zero3d;               // 加速度偏差
    this->gravity = Zero3d;              // 重力加速度
    this->cov = Matrix<double, DIM_STATE, DIM_STATE>::Identity() * INIT_COV;   // 状态 协方差
  };

...
void standard_pcl_cbk(const sensor_msgs::PointCloud2::ConstPtr &msg) {
    ...
    p_pre->process(msg, ptr);                           // 点云预处理，将ros消息类型转为pcl库的类型，还可包括降采样、提取特征点等操作，这里只是简单地根据角度过滤掉了一些点
    lidar_buffer.push_back(ptr);
    time_buffer.push_back(msg->header.stamp.toSec());
    last_timestamp_lidar = msg->header.stamp.toSec();
    mtx_buffer.unlock();

    // condition_variable是C++中的一个线程同步原语，用于在多个线程之间进行条件变量的等待和通知。notify_all()是condition_variable类的一个成员函数，用于通知所有等待在该条件变量上的线程。
    sig_buffer.notify_all();
}
...
// lidar10Hz，imu200Hz，根据lidar时间辍，确定处于该帧lidar时间范围内的imu帧
...
int main(int argc, char **argv) {
    ...
    VD(DIM_STATE) solution;                             // 状态向量
    MD(DIM_STATE, DIM_STATE) G, H_T_H, I_STATE;
    V3D rot_add, t_add;                                 // 更新量
    ...
    signal(SIGINT, SigHandle);      // 退出
    ros::Rate rate(5000);  // 通过rate.sleep()函数控制节点按照指定的帧率运行
    bool status = ros::ok();        // 用于检查ROS节点是否仍在运行。它返回一个布尔值，如果节点仍在运行，则返回true，否则返回false。

    // for Plane Map
    bool init_map = false;
    std::unordered_map<VOXEL_LOC, OctoTree *> voxel_map;    // std::unordered_map是C++标准库中的一个关联容器，它可以存储一组键值对，并且支持快速的查找、插入和删除操作，key是VOXEL_LOC类型，value是OctoTree类型
    ...
    while (status) {
        ...
        if (sync_packages(Measures)) {
            ...
            p_imu->Process(Measures, state, feats_undistort);          // 匀速模型计算估计位姿
            ...
            if (calib_laser) {
                ...
                for (size_t i = 0; i < feats_undistort->size(); i++) {
                    ...
                    double calib_vertical_angle = deg2rad(0.15);           // 角度转弧度，为什么是0.15？？？
            ...
            if (feats_undistort->empty() || (feats_undistort == NULL)) {
                ...
                cout << "FAST-LIO not ready" << endl;         // 一看就是抄的
                continue;
            }
            ...
            if (flg_EKF_inited && !init_map) {
                ...
                Eigen::Quaterniond q(state.rot_end);
                transformLidar(state, p_imu, feats_undistort, world_lidar);    // 利用位姿将输入点云转到世界坐标系下
                std::vector<pointWithCov> pv_list;
                for (size_t i = 0; i < world_lidar->size(); i++) {               // 计算点的协方差
                    pointWithCov pv;
                ...
                buildVoxelMap(pv_list, max_voxel_size, max_layer, layer_size,         // 建立地图
            ...
            for (size_t i = 0; i < feats_down_body->size(); i++) {                        // 计算点的协方差，作用？？？
            ...
            for (iterCount = 0; iterCount < NUM_MAX_ITERATIONS; iterCount++) {
                ...
                transformLidar(state, p_imu, feats_down_body, world_lidar);   // 转到世界坐标系下
                ...
                BuildResidualListOMP(voxel_map, max_voxel_size, 3.0, max_layer, pv_list,          // 匹配
                                     ptpl_list, non_match_list);

                auto scan_match_time_end = std::chrono::high_resolution_clock::now();

                effct_feat_num = 0;
                for (int i = 0; i < ptpl_list.size(); i++) {             // 计算误差
                    PointType pi_body;
                    PointType pi_world;
                    PointType pl;
...
//                ROS_INFO("effct_feat_num: %d ", effct_feat_num);
                scan_match_time +=
                        std::chrono::duration_cast<std::chrono::duration<double>>(
                ...
                if (!flg_EKF_inited) {
                    ...
                    solution = K * meas_vec + vec - K * Hsub * vec.block<6, 1>(0, 0);   // 状态是18维？？？，徐凯老师说滤波器耗内存就是因为H、K的维度与点的数量有关

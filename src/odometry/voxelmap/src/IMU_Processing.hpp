...
void ImuProcess::only_propag(const MeasureGroup &meas, StatesGroup &state_inout,
    ...
    /* covariance propagation */
    // M3D acc_avr_skew;
    M3D Exp_f = Exp(state_inout.bias_g, dt);      // 角速度 x dt = 旋转增量，但是为什么用的是角速度偏差呢？？？
    ...
    state_inout.cov = F_x * state_inout.cov * F_x.transpose() + cov_w;            // 匀速模型，包括旋转、平移和协方差
...
void ImuProcess::Process(const MeasureGroup &meas, StatesGroup &stat,
    ...
    if (imu_en) {
        ...
        cov_acc = Eye3d * cov_acc_scale;
        cov_gyr = Eye3d * cov_gyr_scale;
        only_propag(meas, stat, cur_pcl_un_);   // 用匀速模型，没有反向传播去畸变，只有
//        ROS_INFO("propagate state: \n"
//                 "position: %.8f %.8f %.8f; \n"
//                 "velocity: %.8f %.8f %.8f; \n"
//                 "bias_a: %.8f %.8f %.8f; \n"
//                 "bias_g: %.8f %.8f %.8f; \n"
//                 "gravity: %.8f %.8f %.8f \n",
//                 stat.pos_end[0], stat.pos_end[1], stat.pos_end[2],
//                 stat.vel_end[0], stat.vel_end[1], stat.vel_end[2],
//                 stat.bias_a[0], stat.bias_a[1], stat.bias_a[2],

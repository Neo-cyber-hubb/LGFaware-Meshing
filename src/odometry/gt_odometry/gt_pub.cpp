...
//        bin_point_cloud_path << file_dataset << seq << "/velodyne/" << std::setfill('0') << std::setw(6) << line_num << ".bin";
        bin_point_cloud_path << file_dataset << "/velodyne/" << std::setfill('0') << std::setw(6) << line_num << ".bin";
    }
    else if(dataset == 2){//mai_city
//        bin_point_cloud_path << file_dataset << seq << "/velodyne/" << std::setfill('0') << std::setw(5) << line_num << ".bin";
        bin_point_cloud_path << file_dataset << "/velodyne/" << std::setfill('0') << std::setw(5) << line_num << ".bin";
    }
...
int main(int argc, char** argv) {
    ...
    while (ros::ok()) {
        ...
        if (!readKitti(data_dir, frame_idx, dataset_num, *cloud)) {
            std::cout << "No more data or path error!" << std::endl;
            break;
        }
        ...
        if (dataset_num == 1)
        {
            ...
            pose_T(0, 0) = rotation_matrix(0, 0);
            pose_T(0, 1) = rotation_matrix(0, 1);
            pose_T(0, 2) = rotation_matrix(0, 2);
            pose_T(0, 3) = t(0);
            pose_T(1, 0) = rotation_matrix(1, 0);
            pose_T(1, 1) = rotation_matrix(1, 1);
            pose_T(1, 2) = rotation_matrix(1, 2);
            pose_T(1, 3) = t(1);
            pose_T(2, 0) = rotation_matrix(2, 0);
            pose_T(2, 1) = rotation_matrix(2, 1);
            pose_T(2, 2) = rotation_matrix(2, 2);
            pose_T(2, 3) = t(2);
            pose_T(3, 0) = 0;
            pose_T(3, 1) = 0;
            pose_T(3, 2) = 0;
